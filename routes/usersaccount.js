...
const sendActivationMail=async(userFname,userLname,email,randomstring)=>{
  try {
...
const sendResetMail=async(userFname,userLname,email,randomstring)=>{
  try {
console.log("this is send section");
const transpoter= nodemailer.createTransport({
  host:"smtp-mail.outlook.com",
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (user) {
      ...
      try {
        //Hash the password
        console.log("this is befor hash password");
        ...
        try {
          sendActivationMail(req.body.userFname,req.body.userLname, req.body.email, randstring);
...
res.status(400).send({"message":"Invalid credentials or Account is already active"});
  }
} catch (error) {
...
router.post("/forgot_password", async (req, res) => {
  try {
    ...
    if (user) {
      ...
      sendResetMail(user.userFname,user.userLname, user.email, randomstring);
